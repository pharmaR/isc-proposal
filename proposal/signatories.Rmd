# Signatories
<!-- 
This section provides the ISC with a view of the support received from the community for a proposal. Acceptance isn't predicated on popularity but community acceptance is important.  Willingness to accept outside input is also a good marker for project delivery. 

An optional section would be for R-Core signatories where changes to R are proposed.
-->

## R Validation Hub Governance

Overall accountability for the R Validation Hub initiative project falls to the R Validation Hub Executive Committee.

* Andy Nicholls (Chair)
* Lyn Taylor (Secretary)
* Joseph Rickert  (R Consortium)
* Min Lee (Link to Transcelerate)
* Yilong Zhang (Package Metrics Stream Lead)
* Keaven Anderson (Testing Stream Lead)


## Project team
<!-- 
Who are the people responsible for actually delivering the project if the proposal gets accepted and are already signed up and rearing to go?
-->
This project breaks down into two pieces.  The larger application development piece will be lead by Andy Nicholls.  The collection of metrics will be lead jointly by Yilon Zhang and Douglas Kelkoff (who will also join the Excutive Committee).


## Consulted
<!-- 
Who has been given the opportunity to provide feedback on the proposal? This should include any R Consortium & ISC members who the proposal has been discussed with.
-->
In terms of ISC members, this proposal has been shared with Joseph Rickert and the idea for a package and app has the general support of the R Validation Hub wider community, which also include Alun Bedding.
